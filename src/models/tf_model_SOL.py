...
class structured_output_layer():
    '''Structrued output layuer'''

    def __init__(self, output_size, s_size=65, name="sol"):

        self._output_size = output_size
        self._s_size = s_size

    def _build(self, input_sequence):
        #split sequence into tow parts: lf0(s1) + mgc(s2)
        with tf.variable_scope('SOL'):
            h_s = snt.BatchApply(snt.nets.MLP(output_sizes=[self._s_size], activation=tf.nn.relu, activate_final=True, name="W_hs"))(input_sequence)
            h_p = snt.BatchApply(snt.nets.MLP(output_sizes=[self._output_size - self._s_size], activation=tf.nn.relu, activate_final=True, name="w_hp"))(input_sequence)
            output_seq_s = h_s + snt.BatchApply(snt.nets.MLP( output_sizes=[self._s_size], activation=tf.nn.tanh, activate_final=True, name="sol_activation"))(h_p)
            output_seq_s = snt.BatchApply(snt.Linear(self._s_size))(output_seq_s)
            output_seq_p = snt.BatchApply(snt.Linear(self._output_size - self._s_size))(h_p)
        return tf.concat([output_seq_s, output_seq_p], 2)
...
class TfModel(snt.AbstractModule):
    ...
    def __init__(self, rnn_cell, num_hidden, dnn_depth, rnn_depth, output_size,
        ...
        Args:
            ...
            sol_output: Whether to use SOL(Structured Output Layer).
        ...
        with self._enter_variable_scope():
            ...
            elif self._sol_output:
                self._output_module = structured_output_layer(output_size, 65, name="SOL_output")
            else:
    ...
    def _build(self, input_sequence, input_length):
        ...
        if self._rnn_output:
            ...
            output_sequence_logits = self._output_module._build(output_sequence)
        else:
    ...
    def cost(self, logits, target, length):
        ...
        if not self._mdn_output:
            ...
            # compute lf0_loss and mgc_loss separately, and combine with parameter alpha
            alpha = 0.9
            mgc_logits = tf.slice(logits,[0, 0, 0], [-1, -1, 65])
            mgc_target = tf.slice(target,[0, 0, 0], [-1, -1, 65])
            lf0_logits = tf.slice(logits,[0, 0, 65], [-1, -1, -1])
            lf0_target = tf.slice(target,[0, 0, 65], [-1, -1, -1])
            mgc_loss = tf.reduce_sum(0.5 * tf.square(mgc_logits - mgc_target),axis=[2])
            lf0_loss = tf.reduce_sum(0.5 * tf.square(lf0_logits - lf0_target),axis=[2])
            #loss = tf.reduce_sum(0.5 * tf.square(logits - target), axis=[2])
            loss = alpha * mgc_loss + (1 - alpha) * lf0_loss
            # Mask the loss.
            mask = tf.cast(
